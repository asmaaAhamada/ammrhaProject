...
import { Complaints, BaseUrl } from '../../Api'; // حافظي على الاستيرادات المتاحة لديكِ
import { getData } from '../../ApiServecies';

// تعديل الـ Thunk ليستقبل المعرّف (id) ديناميكياً عند الاستدعاء
export const fetchDetailsComplaints = createAsyncThunk(
  "program/fetchDetailsComplaints",
  async (id, { rejectWithValue }) => {
    try {
      console.log("API CALL START FOR ID:", id);
      const response = await getData(`${BaseUrl}${Complaints}/${id}`);
...
export const counterSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      .addCase(fetchDetailsComplaints.pending, (state) => {
        state.isLoading = true;
        state.error = null;
      })
      .addCase(fetchDetailsComplaints.fulfilled, (state, action) => {
        state.isLoading = false;
        state.data = action.payload.data; // هنا يسند كائن الـ data الداخلي (id, name, created_at...)
      })
      .addCase(fetchDetailsComplaints.rejected, (state, action) => {
        state.isLoading = false;
