...
// دالة مخصصة لجعل الحركة تنزلق من الأعلى للأسفل بشكل لطيف
...
export default function AddSection({ open, onClose, onSuccess }) {
  ...
  // حالات التحكم بظهور وإخفاء التنبيهات
  ...
  // مراقبة حالة النجاح والخطأ القادمة من الـ Redux
  useEffect(() => {
    if (success) {
      setToast({
        open: true,
      ...
      const timer = setTimeout(() => {
        ...
        onClose();
      }, 1500); // زيادة الوقت قليلاً ليرى المستخدم الرسالة بوضوح قبل الإغلاق
  ...
  useEffect(() => {
    if (error) {
      setToast({
        open: true,
        message: typeof error === 'string' ? error : "حدث خطأ ما أثناء الإضافة!",
        severity: "error",
      });
    }
  }, [error]);

  // دالة لإغلاق التنبيه يدوياً أو تلقائياً بعد فترة
  const handleCloseToast = (event, reason) => {
    if (reason === 'clickaway') return;
    setToast((prev) => ({ ...prev, open: false }));
    if (toast.severity === 'error') {
      dispatch(resetForm()); // تصفير الخطأ من الريدوكس عند إغلاق التنبيه
    }
  };

  // تنظيف الروابط المؤقتة لمنع تسريب الذاكرة
  useEffect(() => {
    return () => {
      if (imagePreview) {
  ...
  const handlAdd_Department = () => {
    if (!formInfo.name.trim()) {
      setToast({
        open: true,
        message: "الرجاء إدخال اسم القسم أولاً",
        severity: "error",
      });
      return;
    }
  ...
  return (
    <>
      ...
      <Snackbar
        ...
        anchorOrigin={{ vertical: "top", horizontal: "center" }} // يظهر في أعلى المنتصف
        TransitionComponent={TransitionDown} // حركة السلايد اللطيفة من الأعلى للأسفل
        sx={{ direction: "rtl" }}
      >
        <Alert
          onClose={handleCloseToast}
          severity={toast.severity}
          variant="filled" // يعطي لوناً ممتلئاً وواضحاً متناسقاً مع ثيم التطبيق الافتراضي
          sx={{
            width: "100%",
            borderRadius: "12px",
            fontSize: "14px",
            fontWeight: 600,
            boxShadow: "0px 4px 20px rgba(0, 0, 0, 0.15)", // ظل خفيف ليعطي طابع الطفو فوق العناصر
            fontFamily: "inherit",
            '& .MuiAlert-icon': {
              marginLeft: '12px', // تنسيق المسافة للأيقونة بالـ RTL
              marginRight: 0
            }
          }}
        >
